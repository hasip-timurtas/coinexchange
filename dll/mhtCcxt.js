...
class MhtCcxt {
  ...
  CommonFormat(type, data) {
    switch (type) {
      case 'market':
       return {
          TradePairId: data.info.TradePairId || data.info.MarketID,
          AskPrice: data.ask,
          BidPrice: data.bid,
          High: data.high,
          Label: data.symbol,
          ...
          Volume: this.exchangeId == 'coinexchange' ? data.quoteVolume :  data.bid * data.baseVolume, // coinex ise quote volume değilse bid.vasevolume versin. gerçek volume için
        }
      break;
      case 'balance':
        return {
